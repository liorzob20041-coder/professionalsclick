...
def worker_reviews(lang, worker_id):
    ...
    # אודות: bio_full > description, מנקים שורות שמצהירות על "השירותים כוללים..."
    ...
    # רשימת שירותים לכרטיס "מה אני עושה" (רק bullet-ים)
    ...
    # ניסיון: שורה קצרה בהתאם לשפה
    exp = worker.get('experience')
    if isinstance(exp, (int, float)) and exp:
        years = int(exp)
        if lang == 'he':
            worker['experience_text'] = f"{years} שנות ניסיון"
        elif lang == 'en':
            worker['experience_text'] = f"{years} years of experience"
        else:
            worker['experience_text'] = f"{years} лет опыта"
    else:
        worker['experience_text'] = ''

    # איזורי שירות: מאחדים base_city + service_areas/active_cities (עם שמירת סדר והסרת כפילויות)
    areas_raw = []
    ...
    for r in reviews:
        r['display_text'] = r.get('translations', {}).get(lang) or r.get('text', '')
